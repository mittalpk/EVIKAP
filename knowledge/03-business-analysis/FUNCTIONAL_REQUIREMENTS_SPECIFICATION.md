...
**Acceptance check:** For a query phrased differently from the source document's wording but matching its meaning, the correct source is retrieved in the top-k results. (This directly closes the gap identified in the [audit](../OMEGA_NEXUS_AUDIT.md): current retrieval is keyword/substring only.)
...
**Acceptance check:** A reference external agent can discover and successfully invoke the tool using only the published standard interface contract, with no Omega-Nexus-specific client code.
